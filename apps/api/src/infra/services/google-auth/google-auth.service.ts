...
const { baseUrl, clientId, clientSecret, redirectUri, scope } =
...
export class GoogleAuthService implements PartnerOAuthService {
  ...
  async getToken(authCode: string): Promise<Token> {
    try {
      ...
      if (error.response) {
        throw new Error(
          `Google token error: ${error.response.status} - ${JSON.stringify(error.response.data)}`,
        );
      } else {
  ...
  async refreshToken(token: string): Promise<Token> {
    try {
      ...
      if (error.response) {
        ...
        throw new Error(
          `Google refresh token error: ${error.response.status} - ${JSON.stringify(error.response.data)}`,
        );
      } else {
