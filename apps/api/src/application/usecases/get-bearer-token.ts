  ...
  async execute(bearerToken: string): Promise<string> {
    ...
    if (!user || !user.partner_token) {
      throw new BusinessError(
        `User not found or partner token is missing for UUID: ${tokenPayload.uuid}`,
      );
    }

    const sessionId = user.partner_token;
    const token = (await this.tokenRepository.getTokenByUUID(
      sessionId,
    )) as Token | null;
    if (!token) {
      throw new BusinessError(
        `Token not found for the provided session ID: ${sessionId}`,
      );
    }

    if (this.isTokenExpired(token)) {
      const newToken = await this.saveRefreshToken.execute(
        token.refresh_token,
        token.uuid,
      );
