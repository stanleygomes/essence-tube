---
name: observability
description: Logging conventions using Pino and @packages/logger for all apps in this monorepo.
---
...
| App             | Logger                                                |
| --------------- | ----------------------------------------------------- |
| `apps/auth-api` | `@packages/node-utils` (shared package wrapping Pino) |
| `apps/core-ai`  | `@packages/node-utils` (shared package wrapping Pino) |
...
- [ ] Logger imported from `@packages/logger` (or the app's configured Pino instance)
...
```typescript
// apps/auth-api — using @packages/logger
